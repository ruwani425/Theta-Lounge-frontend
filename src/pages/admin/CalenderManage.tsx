...
interface AppointmentCountResponse {
    ...
    count: number; // Total booked sessions for that day
}
...
const apiService = {
    getSystemSettings: async (): Promise<SystemSettings> => {
        try {
            ...
            return { ...DEFAULT_SYSTEM_SETTINGS, ...response } 
        } catch (error) {
            Swal.fire({
                icon: "error",
    ...
    getAppointmentCounts: async (
        formattedStartDate: string,
        formattedEndDate: string,
    ): Promise<AppointmentCountResponse[]> => {
        try {
            const apiResponse = await apiRequest.get<ApiResponse<AppointmentCountResponse[]>>(
                `${APPOINTMENT_API_BASE_URL}/counts`,
                {
                    params: { startDate: formattedStartDate, endDate: formattedEndDate },
                }
            );
...
const DaySettingsSidebar: React.FC<DaySettingsSidebarProps> = ({
    ...
    const maxSessionsPerTank = useMemo(() => {
        ...
        const currentCycleDuration = systemSettings.sessionDuration + systemSettings.cleaningBuffer;

        if (currentCycleDuration <= 0) return 0;
        ...
        try {
            ...
            if (close.getTime() <= open.getTime()) return 0
            const durationMinutes = (close.getTime() - open.getTime()) / (60 * 1000)
            return Math.floor(durationMinutes / currentCycleDuration) 
        } catch (e) {
            return 0
        }
    }, [openTime, closeTime, systemSettings])
    ...
    const handleSave = async (e: FormEvent) => {
        ...
        if (status === DAY_STATUS.BOOKABLE) {
            ...
            const validationCycleDuration = systemSettings ? systemSettings.sessionDuration + systemSettings.cleaningBuffer : 0;
            if (maxSessionsPerTank <= 0) {
                Swal.fire({
                    icon: "warning",
                    title: "Validation Error",
                    text: `Operating hours must allow at least one full session + cleaning cycle (${validationCycleDuration} minutes).`,
                })
                return
            }
        }
    ...
    const fetchCalendarData = useCallback(
        async (isInitialLoad = false) => {
            ...
            try {
                ...
                // 1. Fetch Overrides AND Appointment Counts concurrently
                const [facilityOverrides, appointmentCounts] = await Promise.all([
                    apiService.getCalendarOverrides(formattedStartDate, formattedEndDate),
                    apiService.getAppointmentCounts(formattedStartDate, formattedEndDate),
                ]);
                
                // Map appointment counts for quick lookup
                const bookedCountMap = appointmentCounts.reduce((acc, curr) => {
                ...
                // Collect promises for immediate SOLD_OUT updates
                ...
                const newCalendarDays: FacilityDayData[] = dates.map((date) => {
                    ...
                    const bookedCountFromAppointments = bookedCountMap[dateKey] || 0; 
                    ...
                    let totalBookedSessions = bookedCountFromAppointments; 
                    ...
                    // The calculated total capacity for the day
                    const calculatedTotalSessions = sessionsPerTank * currentReadyTankCount;

                    // Determine the current sessionsToSell based on status
                    if (status === DAY_STATUS.BOOKABLE) {
                        totalSessionsToSell = calculatedTotalSessions;
                    } else if (status === DAY_STATUS.CLOSED || status === DAY_STATUS.SOLD_OUT) {
                        totalSessionsToSell = facilityRecord?.sessionsToSell || 0; 
                    }
                    
                    // 🛑 FIX APPLIED HERE: Calculate totalAvailableSessions, enforcing a minimum of zero.
                    let totalAvailableSessions = Math.max(0, totalSessionsToSell - totalBookedSessions);
                    
                    if (status === DAY_STATUS.CLOSED) {
                        totalAvailableSessions = 0
                        totalSessionsToSell = 0
                    } else if (totalAvailableSessions === 0 && totalSessionsToSell > 0 && status !== DAY_STATUS.SOLD_OUT) {
                        // Condition Met: Day is full (available=0) and was BOOKABLE, now trigger SOLD_OUT
                        
                        // 1. Update the status locally for immediate display
                        status = DAY_STATUS.SOLD_OUT
                        
                        // 2. If the original status was BOOKABLE (or no record existed), trigger an API update
                        if (facilityRecord?.status === DAY_STATUS.BOOKABLE || !facilityRecord) {
                             const updatePromise = apiService.updateFacilityStatus(
                                dateKey,
                                DAY_STATUS.SOLD_OUT, 
                                openTime,
                                closeTime,
                            ).catch(e => {
                                console.error(`Failed to automatically mark ${dateKey} as SOLD OUT`, e);
                            });
                    ...
                    return {
                        ...
                        cycleDuration: sessionDuration + cleaningBuffer,
                    }
                })

                // Wait for all automatic updates to finish (non-blocking to the main display)
                await Promise.all(updatePromises); 
                
                // If automatic updates ran, refetch data to ensure the calendar table reflects the committed changes
                if (updatePromises.length > 0) {
                   return fetchCalendarData(false); // Recursive call to refresh with new SOLD OUT status
                }
                                    ...
                                    <tr className="border-t border-gray-200">
                                        ...
                                        {calendarDays.map((dayData, index) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    {/* This will now display 0 if the calculated value was negative */}
